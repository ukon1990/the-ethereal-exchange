  ...
  template: `
    ...
    >
      @for (toast of toasts.messages(); track toast.id) {
        <div
          class="pointer-events-auto rounded border border-error/40 bg-surface-container-high px-4 py-3 text-sm text-on-surface shadow-xl"
          role="alert"
        >
          <div class="flex items-start gap-3">
            <span class="mt-0.5 h-2 w-2 shrink-0 rounded-full bg-error"></span>
